...
void AfcScrollArea::wheelEvent(QWheelEvent *event)
{
    if((event->modifiers() & Qt::ControlModifier) != 0) {
        event->ignore();
#if QT_VERSION >= 0x060000
        emit zoomStepped(event->angleDelta().y() / 120);
#else
...
{
    // Generator files are loaded directly via slotReloadGenerators() and generateCode()
    // using built-in directory fallback paths (local dir, then installed dir).
...
void MainWindow::retranslateUi()
{
    ...
    actImportC->setText(tr("Import &C source..."));
    actImportC->setStatusTip(tr("Build flowchart from C source code"));
    actImportPseudo->setText(tr("Псевдоязык"));
    actImportPseudo->setStatusTip(tr("Build flowchart from AFCE Pseudo Language (PlantUML-like syntax)"));
    actImportPseudo->setToolTip(tr("Импорт псевдоязыка APL\nВставьте код в стиле PlantUML для построения блок-схемы по ГОСТ 19.701-90"));
    ...
    helpPaths << qApp->applicationDirPath() + "/help/" + QLocale().name()
              << qApp->applicationDirPath() + "/help/en_US"
              << "./help/" + QLocale().name()
              << "./help/en_US";
#if defined(Q_WS_X11) || defined(Q_OS_LINUX)
    helpPaths << QString(PROGRAM_DATA_DIR) + "help/" + QLocale().name()
              << QString(PROGRAM_DATA_DIR) + "help/en_US";
#endif
    helpWindow->textBrowser->setSearchPaths(helpPaths);
...
void MainWindow::createMenu()
{
    ...
    menuFile->addSeparator();
    menuFile->addAction(actImportC);
    menuFile->addAction(actImportPseudo);
    menuFile->addSeparator();
...
void MainWindow::createToolBar()
{
    ...
    toolBar->addSeparator();
    toolBar->addAction(actImportPseudo);
    toolBar->addSeparator();
...
void MainWindow::createActions()
{
    ...
    actImportC = new QAction(QIcon(":/images/open_document_32_h.png"), "", this);
    actImportPseudo = new QAction(QIcon(":/images/source-code.png"), "", this);

      connect(actExit, SIGNAL(triggered()), this, SLOT(close()));
      connect(actImportC, SIGNAL(triggered()), this, SLOT(slotImportCSource()));
      connect(actImportPseudo, SIGNAL(triggered()), this, SLOT(slotImportPseudoLang()));
...
void MainWindow::slotReloadGenerators()
{
    ...
    // Build list of directories to search for generator JSON files
    QStringList genDirs;
    genDirs << qApp->applicationDirPath() + "/generators"
            << "./generators";
#if defined(Q_WS_X11) || defined(Q_OS_LINUX)
    genDirs << QString(PROGRAM_DATA_DIR) + "generators";
#endif

    QStringList gens;
    QDir gd;
    for (const QString &dir : genDirs) {
        gd.setPath(dir);
        QStringList found = gd.entryList(QStringList() << "*.json", QDir::Files, QDir::Name);
        if (!found.isEmpty()) {
            gens = found;
            break;
        }
...
void MainWindow::slotFileExport()
{
    ...
    if(!fn.isEmpty())
    {
        ...
        QStringList masks;
        auto match = rx.match(sf);
        if(match.hasMatch()) {
            // each filter may content several masks splitted by a spaces
            masks = match.captured(1).split(" ", Qt::SkipEmptyParts);
...
void MainWindow::slotImportCSource()
{
    // Create a dialog for pasting C source code
    QDialog dlg(this);
    dlg.setWindowTitle(tr("Import C Source Code"));
    dlg.resize(600, 400);

    QVBoxLayout *layout = new QVBoxLayout(&dlg);

    QLabel *label = new QLabel(tr("Paste C source code below. The parser will build a flowchart from it."));
    ...
    // Parse and load
    QString error;
    QDomDocument doc = SourceCodeParser::parse(sourceCode, &error);

    if (!error.isEmpty()) {
        qWarning() << "C source parse warning:" << error;
    }

    // Serialize XML and load into editor
    QString xml = doc.toString(2);
    qDebug().noquote() << "Generated XML:" << xml;

    document()->setBuffer(xml);
...
// ---------------------------------------------------------------------------
// slotImportPseudoLang — import pseudo-language (APL) and build flowchart
// ---------------------------------------------------------------------------
void MainWindow::slotImportPseudoLang()
{
    // Create a dialog for entering pseudo-language code
    QDialog dlg(this);
    dlg.setWindowTitle(tr("Import AFCE Pseudo Language"));
    dlg.resize(700, 500);

    QVBoxLayout *layout = new QVBoxLayout(&dlg);

    QLabel *label = new QLabel(tr(
        "Paste AFCE Pseudo Language (APL) code below.\n"
        "Syntax: @start_scheme ... @stop_scheme\n"
        "See PSEUDOLANG.md for full documentation."
    ));
    label->setWordWrap(true);
    layout->addWidget(label);
    ...
    editor->setLineWrapMode(QTextEdit::NoWrap);
    editor->setPlainText(
        QStringLiteral("@start_scheme\n")
        + QStringLiteral("  process \"Hello, World!\";\n")
        + QStringLiteral("@stop_scheme\n")
    );
    layout->addWidget(editor);
    ...
    QString pseudoCode = editor->toPlainText();
    if (pseudoCode.trimmed().isEmpty())
        return;

    // Parse pseudo-language into AFCE XML
    QString error;
    QDomDocument doc = PseudoLangParser::parse(pseudoCode, &error);

    if (!error.isEmpty()) {
        qWarning() << "Pseudo-language parse warning:" << error;
    }
...
void MainWindow::generateCode()
{
    if (document() && codeLanguage->currentIndex() >= 0)
    {
        ...
        QString ruleName = codeLanguage->itemData(codeLanguage->currentIndex()).toString() + ".json";
        QString rulePath;
        for (const QString &dir : genDirs) {
            QFileInfo fi(dir + "/" + ruleName);
            if (fi.exists()) {
                rulePath = fi.absoluteFilePath();
                break;
            }
        }
...
QHash<QString, QString> MainWindow::enumLanguages()
{
    ...
    localeDirs << qApp->applicationDirPath() + "/locale"
               << "./locale";
#if defined(Q_WS_X11) || defined(Q_OS_LINUX)
    localeDirs << QString(PROGRAM_DATA_DIR) + "locale";
#endif
    QDir dir;
    QStringList qms;
    for (const QString &ld : localeDirs) {
        dir.setPath(ld);
        QStringList found = dir.entryList(QStringList() << "afce_*.qm", QDir::Files, QDir::Name);
        qms.append(found);
        if (!qms.isEmpty()) break;
    }

    QRegularExpression rx("^afce_([-_a-zA-Z]+)\\.qm$");
    for (int i = 0; i < qms.size(); ++i) {
        auto match = rx.match(qms[i]);
        if(match.hasMatch()) {
            QString localeName = match.captured(1);
...
void setApplicationLocale(const QString &localeName)
{
    ...
    // Build locale directory list with local paths first, then installed path
    QStringList localeDirs;
    localeDirs << qApp->applicationDirPath() + "/locale"
               << "./locale";
#if defined(Q_WS_X11) || defined(Q_OS_LINUX)
    ...
    for (const QString &dir : localeDirs) {
        if (!qtLoaded)
            qtLoaded = qtTranslator->load(locale, "qt", "_", dir, ".qm");
        if (!appLoaded)
            appLoaded = myappTranslator->load("afce_" + locale.name() + ".qm", dir);
        if (qtLoaded && appLoaded) break;
    }
